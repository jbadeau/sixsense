...
public class IssueLinkJsonParserV5 implements JsonObjectParser<IssueLink> {
    ...
    public IssueLink parse(JSONObject json) throws JSONException {
        ...
        final IssueLinkType issueLinkType = new IssueLinkType(direction, issuelinksType.getName(),
                direction.equals(Direction.INBOUND) ? issuelinksType.getInward()
                        : issuelinksType.getOutward());
        return new IssueLink(key, targetIssueUri.toString(), issueLinkType);
    }
}
