...
public class AttachmentJsonParser implements JsonObjectParser<Attachment> {
    ...
    public Attachment parse(JSONObject json) throws JSONException {
        ...
        user.setAccountId(author.getAccountId());
        user.setDisplayName(author.getDisplayName());
        user.setSelf(author.getSelf().toString());

        return new Attachment(filename, selfUri.toString(), user, Instant.ofEpochMilli(creationDate.getMillis()).atZone(ZoneId.systemDefault()).toLocalDateTime(), size, mimeType, contentURI.toString(), thumbnailURI != null ? thumbnailURI.toString() : null);
    }
}
