...
public class CimIssueTypeJsonParser implements JsonObjectParser<CimIssueType> {
    ...
    public CimIssueType parse(final JSONObject json) throws JSONException {
        ...
        try {
            return new CimIssueType(new URI(issueType.getSelf().toString()), issueType.getId(), issueType.getName().toString(),
                    issueType.getIsSubtask(), issueType.getDescription().toString(), new URI(issueType.getIconUri().toString()), fields);
