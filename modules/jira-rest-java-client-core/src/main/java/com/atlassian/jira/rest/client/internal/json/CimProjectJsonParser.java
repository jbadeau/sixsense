...
public class CimProjectJsonParser implements JsonObjectParser<CimProject> {
    ...
    public CimProject parse(final JSONObject json) throws JSONException {
        ...
        try {
            return new CimProject(new URI(basicProject.getSelf().toString()), basicProject.getKey().toString(), basicProject.getId(),
                    basicProject.getName().toString(), avatarUris, issueTypes);
