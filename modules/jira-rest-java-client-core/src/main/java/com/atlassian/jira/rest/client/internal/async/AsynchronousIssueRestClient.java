...
public class AsynchronousIssueRestClient extends AbstractAsynchronousRestClient implements IssueRestClient {
    ...
    public Promise<Iterable<Transition>> getTransitions(final Issue issue) {
        if (issue.getTransitionsUri() != null) {
            try {
                return getTransitions(new URI(issue.getTransitionsUri().toString()));
            } catch (URISyntaxException e) {
                e.printStackTrace();
                return null;
            }
        } else {
            final UriBuilder transitionsUri = UriBuilder.fromUri(issue.getSelf().toString());
    ...
    public Promise<Void> transition(final Issue issue, final TransitionInput transitionInput) {
        if (issue.getTransitionsUri() != null) {
            try {
                return transition(new URI(issue.getTransitionsUri().toString()), transitionInput);
            } catch (URISyntaxException e) {
                e.printStackTrace();
                return null;
            }
        } else {
            final UriBuilder uriBuilder = UriBuilder.fromUri(issue.getSelf().toString());
