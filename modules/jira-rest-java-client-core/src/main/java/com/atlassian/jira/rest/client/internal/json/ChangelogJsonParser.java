...
import com.atlassian.jira.server.rest.client.api.domain.ChangelogGroup;
...
public class ChangelogJsonParser implements JsonObjectParser<ChangelogGroup> {
    ...
    public ChangelogGroup parse(JSONObject json) throws JSONException {
        ...
        return new ChangelogGroup(user, Instant.ofEpochMilli(created.getMillis()).atZone(ZoneId.systemDefault()).toLocalDateTime(), new ArrayList<>(items));
    }
}
