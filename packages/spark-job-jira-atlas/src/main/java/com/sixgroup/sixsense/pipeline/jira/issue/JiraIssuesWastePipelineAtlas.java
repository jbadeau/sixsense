...
public class JiraIssuesWastePipelineAtlas {
    ...
    private static void writeBronze(Dataset<Row> ds) {
        ds.write()
                .format("delta")
                .option("mergeSchema", "true")
                .option("checkpointLocation", System.getenv("SPARK_CHECKPOINT_LOCATION"))
                .mode(SaveMode.Append)
                .option("processName", System.getenv("DELTA_TABLE_JIRA_ISSUE_BRONZE"))
                .save(System.getenv("MINIO_BUCKET") + "/" + System.getenv("DELTA_TABLE_JIRA_ISSUE_BRONZE"));
    }
    ...
    private static void writeSilver(Dataset<Row> ds) {
        ds.write()
                .format("delta")
                .option("mergeSchema", "true")
                .option("checkpointLocation", System.getenv("SPARK_CHECKPOINT_LOCATION"))
                .mode(SaveMode.Append)
                .option("processName", System.getenv("DELTA_TABLE_JIRA_ISSUE_SILVER"))
                .save(System.getenv("MINIO_BUCKET") + "/" + System.getenv("DELTA_TABLE_JIRA_ISSUE_SILVER"));
    }
    ...
    private static void writeGold(Dataset<Row> ds) throws TimeoutException {
        ds.write()
                .format("kafka")
                .mode(SaveMode.Append)
                .option("processName", "JIRA_ISSUES_GOLD")
                .option("kafka.bootstrap.servers", System.getenv("KAFKA_BOOTSTRAP_SERVER"))
                .option("kafka.group.id", System.getenv("KAFKA_GROUP_ID"))
